...
describe('AuditReport 组件', () => {
  ...
  it('应该在 loading 时显示加载状态', () => {
    ...
    // 验证加载状态显示（新 UI 使用新的加载组件）
    expect(screen.getByText(/赛博编辑部正在审计/)).toBeInTheDocument();
  });
  ...
  it('应该处理没有批评意见的情况', () => {
    ...
    // 验证"暂无批评意见"提示（新 UI 使用 emoji）
    expect(screen.getByText('太棒了！暂无批评意见')).toBeInTheDocument();
  });
  ...
  it('应该处理没有建议的情况', () => {
    ...
    // 验证"暂无建议"提示（新 UI 使用 emoji）
    expect(screen.getByText('文章已很完善，暂无建议')).toBeInTheDocument();
  });
